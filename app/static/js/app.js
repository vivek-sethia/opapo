...
$(document).ready(function() {
    ...
    $.getJSON("../data.json", function(data) {
        ...
        var star, price_arr, ratings = [], amazon_avg_rating, ebay_avg_rating, ratings2 = [], rakuten_avg_rating;
        rating_stats = {
            'amazon': {'renderTo': 'amazon_rating_stats', 'data': []},
            'ebay': {'renderTo': 'ebay_rating_stats', 'data': []},
            'rakuten': {'renderTo': 'rakuten_rating_stats', 'data': []}
        },
            sentiments = {
                ...
                'rakuten': {'renderTo': 'rakuten_sentiment_chart', 'data': data.rakuten.overall_sentiment || 0}
            },
            tone_details = {
                'amazon': data.amazon.tones || {},
                'ebay': data.ebay.tones || {},
                'rakuten': data.rakuten.tones || {}
            };
        console.log(data);
        ...
        price_arr = (data.rakuten.price).split(".");

        $('#rakuten_price').text(price_arr[0]);
        $('#rakuten_price_cent').text(price_arr[1]);
        $('#rakuten_shipping_cost').text(data.rakuten.shippingCost);

        if(isInStock(data.rakuten.availability || '')) {
            $('#rakuten_availability').attr('src', '../static/img/yes.png');
        }

        if(data.rakuten.review_count)
            $('#rakuten_review_count').text(data.rakuten.review_count + ' reviews');


        $('#rakuten_merchant_feedback').text(data.rakuten.merchant);
        $('#rakuten_product_img').attr('src', data.rakuten.image);
        $('#rakuten_product_link').attr('href', data.rakuten.product_link);

        if(data.rakuten.rating.average) {
            rakuten_avg_rating = (parseFloat(data.rakuten.rating.average.trim('out of 5 stars'))/5)*100;
            $('.rakuten_features .star-ratings-sprite-rating').css('width', rakuten_avg_rating + '%');
        }
        ...
        if(data.rakuten.rating.stats) {
            /*Format the ratings as per data format of charts*/
            $.each(data.rakuten.rating.stats, function(star, rating) {
                rating_stats['rakuten'].data.push({
                    name: star,
                    y: parseFloat(rating)
                })
            });
